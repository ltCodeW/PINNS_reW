...
class PhysicsInformedNN:
    def __init__(self, X,Y,Z,NQ,U,V,W,P,Rho,a,H,Ma,r,A,onWall,layers,ifContinue):
        ...
        self.traindoc = "./noSuperviseModelResult/"
        self.trainProgressfile =open('./noSuperviseModelResult/trainProgress.txt',mode='w')


        if(ifContinue == False):
            self.weights, self.biases = self.initialize_NN(layers)
        else:
            self.weights, self.biases = self.loadPara("fileName")
        ...
        with tf.name_scope('input'):
            ...
            self.a_tf = tf.placeholder(tf.float32, shape=[None, self.a.shape[1]])
            self.H_tf = tf.placeholder(tf.float32, shape=[None, self.H.shape[1]])
            self.Ma_tf = tf.placeholder(tf.float32, shape=[None, self.Ma.shape[1]])
        ...
        with tf.name_scope('lossConstrain'):
        ...
        self.P_pred, self.Rho_pred= self.net_NS(self.X_tf, self.Y_tf,self.Z_tf,
                               )
    ...
    def neural_net(self, X, weights, biases):
        with tf.name_scope('HIDDEN'):
            ...
            for l in range(0, num_layers - 2):
                ...
                varname = "hidden"+str(l)
                tf.summary.scalar(varname,  tf.reduce_mean(H))

            W = weights[-1]
            b = biases[-1]
            Y = tf.add(tf.matmul(H, W), b)
            tf.summary.scalar('Y', tf.reduce_mean(H))
            return Y
    ...
    def net_NS(self,X,Y,Z):
        # preVal = self.neural_net(tf.concat([X,Y,Z,a,H,Ma,r,A],1), self.weights, self.biases)
        ...
        #
        # U = tf.gradients(psi, X)[0]  # psi关于y的导数
        # # v = -tf.gradients(psi, y)[0]  # psi关于x的导数，存疑为何要负
        # V = tf.gradients(psi, Y)[0]
        # W = tf.gradients(psi, Z)[0]
        #
    ...
    def callback(self, loss):
        print('Loss: %.3e ' % loss)
        self.trainProgressfile.write('Loss: %.3e\n' % loss)
    ...
    def train(self, nIter):
        tf_dict = {self.X_tf: self.X,self.Y_tf: self.Y, self.Z_tf: self.Z,
                   ...
                   self.A_tf: self.A, self.H_tf: self.H, self.Ma_tf: self.Ma,
                   self.r_tf: self.r, self.a_tf: self.a, self.onWall_tf: self.onWall
                   }
        ...
        for it in range(nIter):
            ...
            if it % 10 == 0:
                ...
                self.trainProgressfile.write('It: %d, Loss: %.3e, Time: %.2f' %
                                             (it, loss_value, elapsed))
                ...
                print('U: %.3e, V: %.3e , W: %.3e, P: %.3e,'
                      ' Rho: %.3e, ' %
                      (lU, lV, lW, lP, lRho))
                self.trainProgressfile.write('U: %.3e, V: %.3e , W: %.3e, P: %.3e,'
                      ' Rho: %.3e, ' %
                      (lU, lV, lW, lP, lRho))
                start_time = time.time()
    ...
    def predict(self, X,Y,Z):
        ...
        tf_dict = {self.X_tf: X, self.Y_tf: Y, self.Z_tf: Z,
                   }
        ...
        self.trainProgressfile.close()
        return U_star,V_star,W_star,P_star,Rho_star
    def savePara(self):
        self.saver.save(self.sess, './noSuperviseModelResult/1.ckpt', global_step=1)
    def loadPata(self):
        ckpt = tf.train.get_checkpoint_state("./noSuperviseModelResult/")
...
if __name__ == "__main__":
    datafile = "A__bluntcone_a-05_H20km_Ma05_0.00086mm\A00.npy"

    data = loadData(datafile)
    basevar = [5,20,5,0.00086]

    # layers = [8, 20, 20, 20,3]
    # It: 4740, Loss: nan, Time: 0.22
    # U: nan, V: nan, W: nan, P: nan, Rho: nan,
    # layers = [8, 20, 3]
    # It: 140, Loss: nan, Time: 0.41
    # U: nan, V: nan, W: nan, P: nan, Rho: nan,
    # layers = [8, 20, 20, 20,20, 20, 20, 3]
    # It: 140, Loss: nan, Time: 0.41
    # U: nan, V: nan, W: nan, P: nan, Rho: nan,
    ...
    a = [basevar[0]] * Nf_data
    H = [basevar[1]] * Nf_data
    Ma = [basevar[2]] * Nf_data
    r= [basevar[3]] * Nf_data
    A= [basevar[4]] * Nf_data
    X,Y,Z,NDimQ,U,V,W,P,Rho,a,H,Ma,r,A,onWall= map(npformat,[X,Y,Z,NDimQ,U,V,W,P,Rho,a,H,Ma,r,A,onWall])
...
#     获得训练数据,训练数据选取4/5
    ...
    P_train,Rho_train,\
    a_train,H_train,Ma_train,r_train,A_train,onWall_train\
        =map(getBaseVar,[idx]*15,[X,Y,Z,NDimQ,U,V,W,P,Rho,a,H,Ma,r,A,onWall])
    ...
    testidx = np.array(list(testSet))


    X_test, Y_test, Z_test, Q_test, \
    U_test, V_test, W_test, P_test, Rho_test, \
    a_test, H_test, Ma_test, r_test, A_test =\
    map(getBaseVar, [testidx]* 14, [X,Y,Z,NDimQ,U,V,W,P,Rho,a,H,Ma,r,A])

    U_pred,V_pred,W_pred,P_pred,Rho_pred=\
