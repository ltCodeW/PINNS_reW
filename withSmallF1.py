...
# Nslayer层的问题，全连接层的值不是nan，加上内层后变为nan
# 在|pre-true|的基础上上如loss约束
# 加入l1,无nan出现
# 没有预测Q
...
class PhysicsInformedNN:
    def __init__(self, X,Y,Z,NQ,U,V,W,P,Rho,onWall,layers):
        ...
        self.onWall = onWall
        self.layers = layers
        self.traindoc = "./withSmallF1modelResult/"
        self.trainProgressfile = open('./withSmallF1modelResult/trainProgress.txt', mode='w')
        ...
        self.U_pred, self.V_pred, self.W_pred, \
        self.P_pred, self.Rho_pred,\
        self.f_1= self.net_NS(self.X_tf, self.Y_tf, self.Z_tf,
                          self.onWall_tf)
    ...
    def savePara(self):
        self.saver.save(self.sess, self.traindoc+"1.ckpt", global_step=1)
    def loadPata(self):
        ckpt = tf.train.get_checkpoint_state(self.traindoc)
...
#     获得训练数据,训练数据选取4/5
    ...
    #  =  map(getBaseVar, list(testSet) * 14, [X,Y,Z,NDimQ,U,V,W,P,Rho])
    X_test, Y_test, Z_test, Q_test, \
    U_test, V_test, W_test, P_test, Rho_test, \
        = map(getBaseVar, list(testSet) * 9, [X, Y, Z, NDimQ, U, V, W, P, Rho])
